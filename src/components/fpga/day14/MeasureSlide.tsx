...
// coverage report — 기본 TB 측정 결과(요약)
const SUMMARY: { label: string; hit: number; tot: number }[] = [
  { label: 'Statement', hit: 18, tot: 20 },
  { label: 'Branch', hit: 6, tot: 12 },
  { label: 'Condition', hit: 3, tot: 7 },
  { label: 'FSM state', hit: 4, tot: 4 },
  { label: 'FSM tran', hit: 3, tot: 6 },
  { label: 'Toggle', hit: 14, tot: 24 },
];

const HOLES = [
  'branch : else if(en) 의 false (en=0) 미도달',
  'branch : WARN if(!vote) true (회복) 미도달',
  'branch : LATCH if(clear) true 미도달',
  'cond   : vote 곱항 일부 미구분 (sensor=111 만 인가)',
  'fsm    : WARN→MONITOR · LATCH→MONITOR 천이 0',
];
    ...
    <section data-background-color={slideBg}>
      <div className="fpga-content-wrap">
        <SlideHeader
          ...
          subtitle="기능 PASS 여도 커버리지는 낮음 · 숫자에서 &ldquo;안 해본 자극&rdquo; 판독 (DUT: trip_ctrl)"
        />
              ...
              {view === 'summary' ? (
                <>
                  <div style={{ fontSize: '0.58rem', color: '#7C90B0', fontFamily: MONO, marginBottom: '0.4rem' }}>
                    # coverage report — /tb_trip/dut (trip_ctrl)
                  </div>
                  {SUMMARY.map((r) => {
                    const p = pct(r.hit, r.tot);
                    const c = tone(p);
                    return (
                      <div key={r.label} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '0.16rem 0' }}>
                        <span style={{ fontSize: '0.62rem', fontFamily: MONO, color: '#C7D2E8', width: '86px', flexShrink: 0 }}>{r.label}</span>
                        <div style={{ flex: 1, height: '11px', background: '#243250', borderRadius: '6px', overflow: 'hidden' }}>
                          <div style={{ width: `${p}%`, height: '100%', background: c, borderRadius: '6px' }} />
                        </div>
                        <span style={{ fontSize: '0.62rem', fontFamily: MONO, fontWeight: 800, color: c, width: '78px', textAlign: 'right', flexShrink: 0 }}>
                          {p}% <span style={{ color: '#5A6B87', fontWeight: 400 }}>{r.hit}/{r.tot}</span>
                        </span>
                      </div>
                    );
                  })}
                  <div style={{ marginTop: 'auto', paddingTop: '0.4rem', fontSize: '0.58rem', color: '#7C90B0', fontFamily: MONO }}>
