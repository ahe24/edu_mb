...
const categories: CategoryData[] = [
  ...
  },
  {
    ...
    description: '프로젝트 코딩 규칙 및 스타일 가이드라인 위반을 검출합니다. Questa Lint FPGA methodology는 비동기 active-high 리셋의 존재를 INFO로 알립니다. "sync/active-high 강제" 검사는 없으며, 이는 설계 리뷰 및 프로젝트 정책으로 관리합니다.',
    ruleId: 'async_reset_active_high',
    ruleName: 'async_reset_active_high',
    file: 'reset_flop.v',
    codeLines: [
      { text: "always @(posedge Clk or posedge Rst) begin", highlight: true, annotate: "INFO: async reset detected — active-high polarity noted" },
      { text: "    if (Rst)" },
      { text: "        Q <= 8'h00;" },
      { text: "    else" },
      { text: "        Q <= D;" },
    ...
    detail: "Questa Lint FPGA methodology가 비동기(async) 리셋이 active-high 극성임을 INFO로 알립니다. Active-high 극성 자체는 FPGA(Xilinx)에 올바른 선택입니다 — FPGA 내부 FF의 리셋 핀이 하드웨어적으로 active-high이기 때문입니다. 단, async 리셋 해제(de-assertion) 시 클럭 엣지와 겹치면 메타스테빌리티가 발생할 수 있어 설계 검토가 필요합니다.",
    howToFix: "극성(active-high)은 FPGA에 적합. 필요 시 reset de-assertion synchronizer 추가로 async 해제 타이밍 보호",
  },
];
