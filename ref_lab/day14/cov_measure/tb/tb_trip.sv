...
//   목적 : 최소 자극으로 trip_top 을 돌려 "PASS 인데 커버리지는 낮다"를 관찰.
//   자극 : en=1 고정, sensor=3'b111 지속 인가 → WARN 누적 → TRIP → LATCH.
//   약점 : clear 없음, en=0 없음, 일시 초과(transient) 없음, sensor 조합 단조.
//          → vote2oo3/warn_counter/trip_fsm 세 서브모듈에 각각 홀이 남음(실습2 에서 보강).
...
module tb_trip;
    ...
    // DUT ― trip_top (vote2oo3 + warn_counter + trip_fsm)
    ...
    initial begin
        ...
        check(trip === 1'b1,           "trip 은 지속 초과 시 작동해야 한다");
